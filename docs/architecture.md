...
**路线顺序**：`route` 存在时，每个策略的**确定性 pass（ρ=0，首次迭代）**追加一趟稳定排序，按平台 route 深度（route 索引，越大越深处）降序——深处平台先放、占满 X 小侧，等深度内保持各策略原顺序。这保证深重负载（如 `6p2+4p3` 单容器）能被确定性装入（10/10），且任何策略下近门箱都不会先占死深处。**Shaw 迭代（ρ>0）保持各策略原排序**，配合硬门 `check_route_order` 输出恒合法——这条多样性（体积优先的容器分配、可合并布局）是 platform_merge / min_platform 等场景稳定通过的关键，因此无需独立的路线策略。
...
体积率优先，并列时剩余平台数更少者优（对齐目标 `min_platform_split`，倾向平台聚拢，降低后处理救不回的拆分概率）。
...
| 目标优化 | 后处理完成              | 装载层多目标打分 + 后处理完成      | 体积率+剩余平台数评分 + 后处理完成      | 体积率最大化 + 后处理完成 |

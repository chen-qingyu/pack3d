...
MLHS **没有投影机制**，它的各层使用单一指标做决策，多目标在最终统计时计算：

| 层级                           | 决策指标               | 行为                                                                                                             |
| ------------------------------ | ---------------------- | ---------------------------------------------------------------------------------------------------------------- |
| **全局调度** `GlobalScheduler` | 装箱数量越多越好       | 对每种容器类型调用 `pack_container()` 尝试装下全部剩余箱子，选装得**最多的类型**。间接优化 `min_container_count` |
| **容器内贪心** `pack()`        | 块体积越大越好         | 每次选体积最大的可行块放置，纯贪心无前瞻                                                                         |
| **容器内 Beam** `pack_beam()`  | 模拟填满后的体积利用率 | 对候选块做前瞻评估：模拟放置后用 `greedy_complete()` 填满剩余空间，用最终体积利用率作为 fitness                  |

MLHS 不直接在决策时考虑 `platform_count` 和 `group_split`。这两个维度只在最终 `to_solution()` 中从装箱结果计算出来，仅作为输出报告，不影响搜索过程。
...
| 目标优化 | 每步字典序投影，覆盖全部 4 个维度 | 全局选装最多箱子，容器内选最大块或最优填充率。`platform_count` / `group_split` 仅输出统计，不影响搜索 |

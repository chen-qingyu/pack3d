...
- `Placement` 内部字段 `stack_level` / `col_height` / `col_top_z` / `cum_load` / `supports`（直接支撑下标，均不序列化到输出），由 `constraints.cpp` 的 `check_stack_constraints`（只读预检）、`apply_stack_state`（放置后副作用）、`recompute_stack_state`（任意顺序重建 + 校验，用于续装 / 后处理合并 / 预校验）维护。
- 堆叠状态沿**支撑链**（直接支撑 + 传递支撑）传播：max_stack 每柱层数（`col_height`/`col_top_z`，同层并排不增层）、max_load 面积分摊（A3）+ 整柱累计（`cum_load`），语义与示例见 [constraints.md](constraints.md) 1.8 / 1.9。

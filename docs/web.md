...
1. 在实例工作台点击“新建运行”。
2. 输入运行名称，以及算法、时间限制、支撑率、站点数量限制等求解参数。
3. 在“输入 JSON”区域粘贴 JSON，或导入 `.json` 文件。
4. 点击“格式化”检查并整理 JSON。
5. 点击“开始运行”。

提交前，页面会检查 JSON 语法。输入的结构和业务规则由引擎校验。
...
“输入 JSON”区域的内容与 CLI/SDK 使用同一输入格式（完整字段、朝向枚举与约束见 [input.md](input.md)）。可以直接使用仓库中的 [demo.json](../data/demo.json) 作为测试输入。

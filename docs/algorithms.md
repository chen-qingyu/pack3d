...
- **EP 生成（Alg3/4）**：每次放置后对每个投影方向 j 和每个第二方向 d2≠j 生成种子 `p_j = pos[j]+size[j]`、`p_d2 = pos[d2]+size[d2]`，沿 d2 向 −d2 投影找第一个未被阴影遮挡的已放物品后表面为 EP（无则取箱壁）；另在顶部中心 `(pos.x, pos.y, pos.z+dz)` 无条件加一个 EP（能否堆叠由 `can_place` 判定）。投影前按方向 d2 对已放物品端位置**预排序一次**（Alg4 一次排序复用：3 个有序序列供 6 个投影起点共用，tie-break 按下标升序保证确定）。

...
**层号 + 每柱层数**：每个已放置箱记录 `stack_level`（所在堆柱层号，地板层 = 1）、`col_height`（所在柱从地板起的层数）与 `col_top_z`（所在柱顶面 z）。放置新箱 B（底面 z = `b`）时：

- `B.stack_level = max(直接支撑箱.stack_level) + 1`（无支撑 -> 1）。
- 对**支撑链上每个箱 X**（B 的直接支撑及其传递支撑）：仅当 B 位于 X 的柱顶（`b == X.col_top_z`，即新增一层）时检查：

$$X.col\_height + 1 \le X.max\_stack\_for(X.orientation)$$

放置成功后，对柱顶新层的链上箱 `X.col_height += 1`、`X.col_top_z = B 顶面 z`；`B.col_height = max(直接支撑柱层数 + 新层?1:0)`、`B.col_top_z = B 顶面 z`。直接支撑箱 = 底面贴合（`S.top == B.bottom`）且投影相交（面积 > 0）。

**同层并排不增层**：多个箱子在支撑箱顶面**同一高度**并排摆放时只算一层（第二个及以后 `b < X.col_top_z`，不触发新层检查）。"限堆 N 层"= 柱含自身 N 层。

**示例（跨不同高度支撑，每柱层数 vs 全局层号）**：
...
旧全局层号模型：`B.stack_level = max(1,3)+1 = 4`，与每个支撑比 `4 > S1.max_stack 2` → 误拒。每柱层数：S1 柱 = 1+1 = 2 ≤ 2 ✓；S2 柱 = 3+1 = 4 ≤ 5 ✓ → 放行（S1 头顶实际只有 B 一层）。

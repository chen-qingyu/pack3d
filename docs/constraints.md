...
**零厚膜**：`dx`/`dy`/`dz` 中**恰好一个为 0** 时，障碍物退化为**零厚膜**（垂直于该轴的平面，如隔板/搁板），语义与实体积障碍物不同：

- **纯穿越拦截**：只拦截"严格横跨该平面"的箱子（`pos < 平面 < pos + size`）；贴面（任一侧）均放行。
- **不占容积**：不扣减 `volume_rate` 分母（厚度为 0，体积贡献 0）。
- **不参与支撑**：不充当"顶面等价地板"，也不贡献任何支撑面积。

允许 `dx/dy/dz` 全部为正（实体积障碍物）或恰好一个为 0（零厚膜）；**两个及以上为 0（线/点）非法**。

体积口径：障碍物计入 `volume_rate` 分母（可用容积定义见 [output.md](output.md)）。

预校验（违反 -> `invalid`）：障碍物完全在容器内、障碍物互不重叠（按**正体积交集**判定，零厚膜不与任何障碍物重叠）、`existing_containers` 已有放置与障碍物不重叠、障碍物不侵入斜面楔形禁区（面贴面允许，见 1.12）、障碍物 `dx/dy/dz` **至多一个为 0**。

实现：共享 `check_obstacle`（相交）与 `check_support`（顶面支撑）为四个算法统一兜底；各算法的候选点/空间雕刻策略见 [algorithms.md](algorithms.md)。零厚膜由 `check_obstacle` 逐箱兜底（不雕刻、角点塌缩），同斜面模式。

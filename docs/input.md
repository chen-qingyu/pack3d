...
| `dx`/`dy`/`dz` | int>=0 | 是   | 沿容器轴的实际尺寸；**至多一个为 0**（某个为 0 即零厚膜，见下） |

**零厚膜**：`dx`/`dy`/`dz` 恰好一个为 0 时表示零厚膜（垂直于该轴的平面，如隔板/搁板），只拦截严格横跨该平面的箱子、不占容积、不参与支撑。

预校验：障碍物必须完全在容器内、互不重叠（按正体积交集判定，零厚膜不与任何障碍物重叠）、`dx`/`dy`/`dz` 至多一个为 0、不侵入斜面楔形禁区（面贴面允许）；`existing_containers` 中已有放置不得与障碍物重叠。

### 斜面 `facets`

容器内斜切平面（如航空箱顶部斜切角），所有该类型实例共享。每个斜面用**恰好两个带符号轴名截距**描述，缺失的轴 = 斜面平行贯穿的轴（沿该轴截面不变）；该贯穿轴亦可显式写 `0`，与省略等价（输出恒带三键）：
...
| 字段           | 类型   | 必填     | 说明                                                       |
| -------------- | ------ | -------- | ---------------------------------------------------------- |
| `dx`/`dy`/`dz` | int!=0 | 恰好两个 | 沿该轴的截距：`+` = 从 min 侧向内进深，`-` = 从 max 侧向内 |

斜面切掉的角附近**楔形禁区**箱子不得侵入（面贴面允许）。语义（禁入、不参与支撑、计入体积率分母）详见 [constraints.md](constraints.md) 1.12；斜面之间允许重叠（独立禁区，重叠只是更禁）；障碍物不得侵入斜面楔形禁区。

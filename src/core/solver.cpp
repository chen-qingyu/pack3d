...
Solution SolverEngine::solve_mlhs()
{
    ...
    solution.reason = reason::k_feasible;
    ...
    while (!remaining.empty())
    {
        ...
        if (std::chrono::duration<double>(elapsed).count() >= problem_.time_limit_seconds)
        {
            spdlog::info("Time limit reached, stopping.");
            solution.success = false;
            break;
        }

        // 选容器类型：优先选能装下最多当前剩余箱子的类型
        const ContainerType* best_ct = nullptr;
        PackResult best_result;
        size_t best_packed = 0;

        for (const auto& ct : problem_.container_types)
        {
            // 检查 quantity_limit
            auto usage_it = container_type_usage.find(ct.id);
            int used = (usage_it != container_type_usage.end()) ? usage_it->second : 0;
            if (ct.quantity_limit.has_value() && used >= ct.quantity_limit.value())
            {
                continue;
            }
            ...
            PackResult pr = packer.pack(remaining);

            if (pr.placements.size() > best_packed)
            {
                best_packed = pr.placements.size();
                best_ct = &ct;
                best_result = std::move(pr);
            }
        }

        if (best_ct == nullptr || best_packed == 0)
        {
            // 没有容器能装下任何箱子
            solution.success = false;
            solution.reason = reason::k_no_solution;
            break;
        }

        // 提交这个容器的装载结果
        std::string instance_id = fmt::format("container_{}", next_instance++);
        container_type_usage[best_ct->id]++;
        ...
        cs.used_volume = best_result.used_volume;
        cs.volume_rate = best_ct->inner_size.volume() > 0
                             ? static_cast<double>(best_result.used_volume) / static_cast<double>(best_ct->inner_size.volume())
                             : 0.0;
        cs.max_weight = best_ct->max_weight;
        if (has_weight_info_)
        {
            cs.used_weight = best_result.total_weight;
            if (best_ct->max_weight.has_value() && best_ct->max_weight.value() > 0)
            {
                cs.weight_rate = best_result.total_weight / best_ct->max_weight.value();
            }
        }
        cs.platforms.assign(best_result.platforms.begin(), best_result.platforms.end());
        cs.groups.assign(best_result.groups.begin(), best_result.groups.end());

        solution.container_summaries.push_back(std::move(cs));
        solution.container_placements.push_back(std::move(best_result.placements));

        // 更新剩余箱子（保留未装进去的）
        std::set<std::string> unpacked_set(best_result.unpacked_box_ids.begin(),
                                           best_result.unpacked_box_ids.end());
        std::vector<Box> truly_remaining;
        for (const auto& bx : remaining)
        {
            if (unpacked_set.count(bx.id))
            {
                truly_remaining.push_back(bx);
            }
        }
        remaining = std::move(truly_remaining);

        spdlog::info("Container#{} \"{}\": packed {}, remaining {}",
                     next_instance, best_ct->id,
                     best_packed, remaining.size());
    }

    // 构建统计
    auto elapsed = std::chrono::steady_clock::now() - start_time;
    solution.elapsed_second = std::chrono::duration<double>(elapsed).count();

    solution.packed_box_count = static_cast<int>(problem_.boxes.size() - remaining.size());
    solution.unpacked_box_count = static_cast<int>(remaining.size());
    ...
    // 计算目标
    solution.objective.container_count = static_cast<int>(solution.container_summaries.size());
    solution.objective.platform_count = 0;
    double sum_rate = 0.0;
    for (const auto& cs : solution.container_summaries)
    {
        solution.objective.platform_count += static_cast<int>(cs.platforms.size());
        sum_rate += cs.volume_rate;
    }
    solution.objective.avg_volume_rate = solution.objective.container_count > 0
                                             ? sum_rate / solution.objective.container_count
                                             : 0.0;
    // group_split_sum: Σ(容器数 per group - 1)
    {
        ...
        solution.objective.group_split_sum = split;
    }

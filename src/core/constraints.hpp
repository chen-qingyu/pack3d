...
/// 各自朝向的 max_stack。异型箱不互相计数，**同型连续 run 由 max_stack 快速路径兜底**。
/// max_load（跨型接口才强制）：A3 面积分摊仅对"候选 B 与其直接支撑异型"的对；D 整柱累计
/// 仅对"上方（直接/传递）存在异型箱"的链上箱检查（`Placement::has_cross_above`）。
/// indices 非空时只检查这些 placement 下标（网格加速的支撑带候选超集，结果不变）。

...
function Viewer3D({ containers, pallets = [] }: { containers: ContainerResult[]; pallets: PalletResult[] }) {
    ...
    () =>
      containerIndex < 0
        ...
        : containers[containerIndex]
          ? [{ container: containers[containerIndex], index: containerIndex }]
          : [],
    [containers, containerIndex],
  );
  ...
  useEffect(() => {
    ...
    const categoryValues = visibleEntries.flatMap(({ container }) =>
      container.placements.filter((placement) => !palletMap.has(placement.box_id)).map((placement) => categoryOf(placement, colorMode)),
    );
    ...
    visibleEntries.forEach(({ container, index }) => {
      ...
      container.facets.forEach((facet) => {
        ...
        const positions = new Float32Array([A.x, A.y, A.z, B.x, B.y, B.z, A2.x, A2.y, A2.z, B.x, B.y, B.z, B2.x, B2.y, B2.z, A2.x, A2.y, A2.z]);
        ...
        mesh.add(
          new THREE.LineSegments(
            ...
            new THREE.LineBasicMaterial({ color: '#8c2f22', transparent: true, opacity: 0.6 }),
          ),
        );
        root.add(mesh);
      });
        ...
        if (shell) {
          activePallet.pallet.placements.forEach((inner) => {
            const innerMesh = new THREE.Mesh(
              geometry,
              new THREE.MeshStandardMaterial({
                color: colors.get(categoryOf(inner, colorMode)) ?? '#5bc0eb',
                roughness: 0.7,
                metalness: 0.08,
              }),
            );
  ...
  }, [visibleEntries, colorMode, visibleKeys, activePallet, palletMap]);
  ...
  useEffect(() => {
    ...
    const center =
      selectedContainer && containerIndex >= 0
  ...
  const allCategories = [
    ...new Set(visibleEntries.flatMap(({ container }) => container.placements.map((placement) => categoryOf(placement, colorMode)))),
  ];
  ...
  return (
    <Paper withBorder mt="lg">
        ...
        <Select
          ...
          data={[
            ...
            ...containers.map((container, index) => ({
              value: String(index),
              label: `#${index + 1} · ${container.type_id}`,
            })),
          ]}
        />
        <Select
          label="视角"
          value={view}
          onChange={(value) => value && setView(value as ViewName)}
          data={[
      ...
      <Box pos="relative" h={500}>
        <Box ref={mountRef} h="100%" aria-label="三维装箱结果视图" />
        <Paper pos="absolute" top={12} left={12} p="sm" shadow="sm">
          <Stack gap={4}>
            <Text size="xs" c="dimmed">
              {colorMode === 'type' ? '箱型' : colorMode === 'platform' ? '平台' : '分组'}
            </Text>
            {allCategories.map((category, index) => (
              <UnstyledButton
                key={category}
                onClick={() =>
                  setVisibleKeys((current) => (current.includes(category) ? current.filter((item) => item !== category) : [...current, category]))
                }
              >
                <Group gap="xs" opacity={visibleKeys.length === 0 || visibleKeys.includes(category) ? 1 : 0.35}>
                  <Box w={8} h={8} bg={palette[index % palette.length]} />
        ...
        {hoveredPlacement &&
          (() => {
            ...
            if (pallet)
              return (
                <Paper pos="absolute" right={12} bottom={12} p="sm" shadow="sm" w={260}>
                  <Text size="sm" fw={700}>
                    {pallet.pallet_id} · 托盘
                  </Text>
                  <Text size="xs" c="dimmed">
                    {pallet.type_id} · {pallet.sx} × {pallet.sy} × {pallet.sz}
                  </Text>
                  <Text size="xs">
                    货物堆高 {pallet.used_height} · 承重 {pallet.used_weight} / {pallet.payload}
                  </Text>
                  <Text size="xs">
                    体积率 {(pallet.volume_rate * 100).toFixed(1)}% · 内部 {pallet.placements.length} 箱
                  </Text>
                  <Text size="xs" c="dimmed">
                    点击查看内部细节
                  </Text>
            ...
            return (
              <Paper pos="absolute" right={12} bottom={12} p="sm" shadow="sm" w={260}>
                ...
                <Text size="xs" c="dimmed">
                  {placement.box_type_id} · {placement.orientation}
                </Text>
                <Text size="xs">
                  位置 {placement.x}, {placement.y}, {placement.z}
                </Text>
                <Text size="xs">
                  放置 {placement.dx} × {placement.dy} × {placement.dz}
                </Text>
                <Text size="xs">
                  {placement.platform || '无平台'} · {placement.group || '无分组'}
                </Text>
              </Paper>
            );
          })()}
        {activePallet && (
          <Paper pos="absolute" left={12} bottom={12} p="sm" shadow="sm" w={280}>
            <Text size="sm" fw={700}>
              {activePallet.pallet.pallet_id} · 托盘内部
            </Text>
            <Text size="xs" c="dimmed">
              {activePallet.pallet.type_id} · 尺寸 {activePallet.pallet.sx} × {activePallet.pallet.sy} × {activePallet.pallet.sz} · 堆高{' '}
              {activePallet.pallet.used_height}
            </Text>
            <Text size="xs">
              承重 {activePallet.pallet.used_weight} / {activePallet.pallet.payload} · 体积率 {(activePallet.pallet.volume_rate * 100).toFixed(1)}%
            </Text>
            <Text size="xs">内部 {activePallet.pallet.placements.length} 箱 · 点击托盘外壳退出查看</Text>
          </Paper>
        )}
      </Box>
    </Paper>

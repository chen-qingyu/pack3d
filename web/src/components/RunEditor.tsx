      ...
      <Group justify="space-between" align="flex-end" mb="xl">
        <div>
          ...
          <Text size="sm" c="dimmed" mt={4}>
            输入 JSON 由引擎校验语法与业务规则。
          </Text>
        </div>
        <Group>
          <Button variant="default" leftSection={<Check size={16} />} onClick={onFormat}>
            格式化
          </Button>
          ...
          <Button leftSection={<Play size={16} />} onClick={onRun}>
            开始运行
          </Button>
        </Group>
      </Group>
        ...
        <Grid.Col span={{ base: 12, md: 8 }}>
          <Paper withBorder p="md">
            <Group justify="space-between" mb="sm">
              <div>
                <Title order={3}>输入 JSON</Title>
                <Text size="sm" c="dimmed">
                  引擎会校验 JSON 结构与业务规则。
                </Text>
              </div>
              <ActionIcon variant="subtle" onClick={() => setField('inputText', initialInput)}>
                <RefreshCw size={16} />
              </ActionIcon>
            </Group>
            <Textarea
              ...
              onChange={(event) => setField('inputText', event.target.value)}
              spellCheck={false}
          ...
          <Paper withBorder p="md">
            <Stack>
              ...
              <NumberInput
                ...
                onChange={(v) => setField('supportRate', v)}
                min={0}
                max={1}
                step={0.01}
                decimalScale={2}

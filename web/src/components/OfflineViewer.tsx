      ...
      <Group justify="space-between" align="flex-end" mb="xl">
        <div>
          ...
          <Text size="sm" c="dimmed" mt={4}>
            {fileName || '选择 pack3d 输出 JSON'}
          </Text>
        </div>
        <Button component="label" leftSection={<Upload size={16} />}>
          打开 JSON
          <input
            hidden
            type="file"
      ...
      {!result && (
        <Paper withBorder p="xl">
          <Stack align="center" gap="sm">
            <FileJson size={28} />
            <Text fw={600}>尚未加载结果</Text>
            <Button component="label" variant="default" leftSection={<Upload size={16} />}>
              选择输出 JSON
              <input
                hidden
                type="file"
      ...
      {result && (
        <>
          <SimpleGrid cols={{ base: 1, sm: 2, lg: 4 }} mb="lg">
            <Metric label="平均体积率" value={formatPercent(result.summary.volume_rate)} icon={<Gauge size={17} />} accent />
            <Metric label="使用容器" value={String(result.summary.container_count)} icon={<Archive size={17} />} />
            <Metric
              label="已装 / 未装箱"
              value={`${result.summary.packed_box_count} / ${result.summary.unpacked_box_count}`}
          ...
          <Paper withBorder p="md" mt="lg">
            <Text component="pre" size="xs" style={{ overflow: 'auto', maxHeight: 300 }}>

  ...
  return (
    <Center h="calc(100vh - 64px)">
      <Stack align="center">
        ...
        <Title order={2}>开始一个装箱实例</Title>
        <Text c="dimmed">创建实例后，加载输入并运行求解器。</Text>
        <Button leftSection={<Plus size={17} />} onClick={onCreate}>
          创建第一个实例
        </Button>
      </Stack>
    </Center>
    ...
    <Container size="xl" py="xl">
      <Group justify="space-between" align="flex-end" mb="xl">
        <div>
          ...
          <Title mt="xs">{instance.instance_name}</Title>
          <Text size="sm" c="dimmed" mt={4}>
            创建于 {formatDate(instance.created_at)} · {instance.run_count} 次运行
          </Text>
        </div>
        <Group>
          ...
          <Button leftSection={<Play size={16} />} onClick={onNewRun}>
            新建运行
          </Button>
        </Group>
      </Group>
        ...
        {runs.length ? (
          <Table.ScrollContainer minWidth={720}>
            <Table striped highlightOnHover>
              ...
              <Table.Tbody>
                {runs.map((run) => (
                  <Table.Tr key={run.run_id} onClick={() => onOpenRun(run)} style={{ cursor: 'pointer' }}>
                    <Table.Td>
                      <Text fw={600}>{run.run_name}</Text>
                    ...
                    <Table.Td>{run.summary ? `${run.summary.container_count} 容器 · ${formatPercent(run.summary.volume_rate)}` : '—'}</Table.Td>
                    <Table.Td>
                      <ActionIcon
                        color="red"
                        variant="subtle"

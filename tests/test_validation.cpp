...
TEST_CASE("pre_validate_input 检测 max_stack 需要 max_load", "[validation]")
{
    Problem p;
    p.container_types.push_back({"ct1", {1000, 1000, 1000}, 1000.0, std::nullopt});
    BoxType bt;
    ...
    for (const auto& v : violations)
    {
        if (v.find("max_stack requires max_load") != std::string::npos)
        {
            found = true;
        }
    }
    ...
    // 补 max_load 后不再报该错
    p.box_types[0].max_load = {100.0};
    violations = pre_validate_input(p);
    for (const auto& v : violations)
    {
        REQUIRE(v.find("max_stack requires max_load") == std::string::npos);
    }
}
...
TEST_CASE("pre_validate_input 检测 max_stack/max_load 需要 support_rate", "[validation]")
{
    ...
    bt.allowed_orientations = {Orientation::XYZ};
    bt.max_stack = {2};
    bt.max_load = {100.0}; // max_stack 需配 max_load，此处只测 support_rate 前提
    p.box_types.push_back(bt);
    p.boxes.push_back({"box1", "bt1", 10.0, "", {}});
...
TEST_CASE("pre_validate_input group/platform 箱型级模式并归一化", "[validation]")
{
    ...
    REQUIRE(p.boxes[0].groups == std::set<std::string>{"g1"});
    REQUIRE(p.boxes[0].platform == "P1");
    REQUIRE(p.existing_containers[0].placements[0].groups == std::set<std::string>{"g1"});
    REQUIRE(p.existing_containers[0].placements[0].platform == "P1");

    auto same = source;
    same.existing_containers[0].placements[0].groups = {"g1"};
    ...
    auto conflict = same;
    conflict.existing_containers[0].placements[0].groups = {"g2"};
    ...
    none.boxes[0].groups.clear();
    none.boxes[0].platform.clear();
    none.boxes[0].weight.reset();
    none.existing_containers[0].placements[0].groups = {"g1"};
...
TEST_CASE("pre_validate_input group/platform 箱子级模式允许实例不同", "[validation]")
{
    ...
    p.container_types.push_back({"ct1", {1000, 1000, 1000}, 1000.0, std::nullopt});
    p.box_types.push_back({"bt1", {100, 100, 100}, {Orientation::XYZ}});
    p.boxes.push_back({"box1", "bt1", std::nullopt, "P1", {"g1"}});
    p.boxes.push_back({"box2", "bt1", std::nullopt, "P2", {"g2"}});
